...
let numOfColors = document.getElementById("number-of-colors").value;
...
let numInSolution = document.getElementById("number-in-solution").value;
setBallCount()
/** set number of balls in solution and guess rows to equal settings value */
...
function setSolutionCount() {
  let solutionCount = document.getElementById("solution").children;
  while (solutionCount.length < numInSolution) {
    newBall.classList.add("color-ball");
    solutionCount[0].parentElement.appendChild(newBall);
  }
}
